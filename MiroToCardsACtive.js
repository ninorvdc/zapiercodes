export async function createCardsOnMiroBoard({
    ...
    // Usaremos caracteres especiais que são menos visíveis
    const SYSTEM_MARKER = '\n\n\n\n\n\n⠀⠀⠀[⠀';
    const MARKER_END = '⠀]⠀⠀⠀';
    ...
    function extractSystemId(item) {
      ...
      // Try to find the appropriate end marker
      let idEnd = -1;
      if (usedMarker.includes('<span')) {
        // For HTML span format, we need to find where our actual marker starts
        const actualMarkerStart = content.indexOf('⠀⠀⠀[⠀', idStart);
        if (actualMarkerStart !== -1) {
          const actualIdStart = actualMarkerStart + '⠀⠀⠀[⠀'.length;
          idEnd = content.indexOf('⠀]⠀⠀⠀', actualIdStart);
          if (idEnd === -1) {
            idEnd = content.indexOf('</span>', actualIdStart);
          }
          // Adjust idStart to point to the actual ID
          const idStart = actualIdStart;
          const extractedId = content.substring(idStart, idEnd).trim();
          const originalId = idMapping.get(extractedId) || extractedId;
          return originalId;
        } else {
          idEnd = content.indexOf(']', idStart);
        }
      } else if (usedMarker.includes('\u2800')) {
        // For Braille blank format, look for the special end marker
        idEnd = content.indexOf('\u2800]', idStart);
        if (idEnd === -1) {
          idEnd = content.indexOf(']', idStart);
        }
      } else if (usedMarker.includes('<!--')) {
        // For HTML comments, look for -->
        idEnd = content.indexOf(' -->', idStart);
      } else {
        // For old format, look for ]
        idEnd = content.indexOf(']', idStart);
      }
    ...
    parsedData.nodes.forEach(node => {
      ...
      switch(node.type) {
        ...
        case 'BHQ':
          nodeText = `<b>BHQ: ${node.title || 'Untitled'}</b>`;
          if (node.properties?.description) {
            nodeText += `<br/><br/><b>Description:</b><br/>${node.properties.description}`;
          }
          if (node.properties?.question) {
            nodeText += `<br/><br/><b>Question:</b><br/>${node.properties.question}`;
          }
          break;
    ...
    const calculateLevels = () => {
      ...
      nodesByType.BHQ.forEach(nodeId => {
        const node = nodes.get(nodeId);
    ...
    // ==============================================================
    // CRIAR CARD DE STATUS TEMPORÁRIO
    // ==============================================================
    
    let statusCardId = null;
    
    async function createStatusCard(message) {
      const statusBody = {
        data: {
          content: `🔄 ${message}`,
      ...
      try {
        ...
        if (response.ok) {
          ...
          statusCardId = cardData.id;
          debug.push(`Created status card: ${message}`);
        }
      } catch (err) {
        debug.push(`Failed to create status card: ${err.message}`);
      }
    }
    
    async function updateStatusCard(message) {
      ...
      try {
        await fetchWithZapier(`${MIRO_API_BASE}/shapes/${statusCardId}`, {
          method: 'PATCH',
          headers: miroHeaders,
          body: JSON.stringify(updateBody)
        });
        debug.push(`Updated status: ${message}`);
      } catch (err) {
        debug.push(`Failed to update status card: ${err.message}`);
      }
    }
    
    async function deleteStatusCard() {
      if (!statusCardId) return;
      
      try {
        await fetchWithZapier(`${MIRO_API_BASE}/items/${statusCardId}`, {
          method: 'DELETE',
          headers: miroHeaders
        });
        debug.push('Deleted status card');
        statusCardId = null;
      } catch (err) {
        debug.push(`Failed to delete status card: ${err.message}`);
      }
    }
    ...
    // Criar card de status inicial
    await createStatusCard('Starting synchronization...');
  
    // 1. DELETAR nós obsoletos
    if (toDelete.size > 0) {
      await updateStatusCard(`Deleting ${toDelete.size} obsolete cards...`);
    }
    ...
    if (toUpdate.size > 0) {
      await updateStatusCard(`Updating ${toUpdate.size} existing cards...`);
    }
    for (const nodeId of toUpdate) {
      ...
      const updateBody = {
        data: {
          content: nodeData.label.substring(0, 3800) + `<span style="color: ${fillColor}">${SYSTEM_MARKER}${getSimpleId(nodeId)}${MARKER_END}</span>`
        },
        style: {
          fillColor: fillColor,
          textAlign: "left",
    ...
    // 3. CRIAR novos nós
    if (toCreate.size > 0) {
      await updateStatusCard(`Creating ${toCreate.size} new cards...`);
    }
    ...
    await updateStatusCard('Synchronizing connectors...');
    ...
    // Finalizar e remover card de status
    await updateStatusCard('Sync completed! Cleaning up...');
    await new Promise(resolve => setTimeout(resolve, 1000)); // Mostrar por 1 segundo
    await deleteStatusCard();
